...
class BPlusTree {
 public:
  ...
  BPlusTree(const std::string &node_data_file_name, const std::string &node_trash_file_name,
            const std::string &leaf_data_file_name, const std::string &leaf_trash_file_name) :
      node_file_(node_data_file_name, node_trash_file_name), leaf_file_(leaf_data_file_name, leaf_trash_file_name) {
 ...
 private:
  ...
  static constexpr int MaxNodeSize =
      (BlockSize + sizeof(Key) - sizeof(int) - sizeof(bool)) / (sizeof(int) + sizeof(ValueType));
  static constexpr int MinNodeSize = (MaxNodeSize + 1) / 2;
  static constexpr int MaxLeafSize = (BlockSize - 2 * sizeof(int)) / sizeof(ValueType);
  ...
  struct Node {
    ...
    explicit Node(int size = 0, bool is_leaf = false) : size_(size), is_leaf_(is_leaf), children_(), data_() {}
  };
  ...
  struct Leaf {
    ...
    explicit Leaf(int size = 0, int next = 0) : size_(size), next_(next), data_() {}
  };
