#ifndef TICKET_SYSTEM_SET_H
#define TICKET_SYSTEM_SET_H
...
template<class T, class KeyCompare = Less<T>>
class Set {
 private:
  enum Colour {
    kRed, kBlack
  ...
  struct RBTreeNode {
    ...
    explicit RBTreeNode(const T &data, Colour colour, RBTreeNode *parent = nullptr, RBTreeNode *left = nullptr,
  ...
  Pair<RBTreeNode *, bool> InsertNode(const T &value) {
    if (root_ == nullptr) {
    ...
    while (true) {
      bool is_smaller = KeyCompare()(value, now->data_);
      bool is_bigger = KeyCompare()(now->data_, value);
  ...
  void EraseNode(RBTreeNode *node) {
    ...
    while (true) {
      while (true) {
        ...
        bool is_smaller = (find_substitution || KeyCompare()(node->data_, now->data_));
        now = (is_smaller ? now->left_ : now->right_);
        if (now->colour_ == kRed) {
          break;
        }
      ...
      now = (find_substitution || KeyCompare()(node->data_, now->data_) ? now->left_ : now->right_);
    }
    root_->colour_ = kBlack;
    delete node;
  }

  RBTreeNode *FindNode(const T &key) const {
    RBTreeNode *now = root_;
    while (now != nullptr) {
      bool is_smaller = KeyCompare()(key, now->data_), is_bigger = KeyCompare()(now->data_, key);
      if (!is_smaller && !is_bigger) {
        return now;
      }
      now = (is_smaller ? now->left_ : now->right_);
 ...
 public:
  ...
  class iterator {
   private:
    friend Set<T, KeyCompare>;

    iterator(RBTreeNode *node, Set<T, KeyCompare> *set_ptr) : node_(node), set_ptr_(set_ptr) {}

    RBTreeNode *node_;
    const Set<T, KeyCompare> *set_ptr_;

   public:
    iterator() : node_(nullptr), set_ptr_(nullptr) {}

    iterator(const iterator &other) : node_(other.node_), set_ptr_(other.set_ptr_) {}
    ...
    bool operator==(const iterator &rhs) const {
      return set_ptr_ == rhs.set_ptr_ && node_ == rhs.node_;
    }
  ...
  class const_iterator {
   private:
    ...
    const_iterator(RBTreeNode *node, const Set<T, KeyCompare> *set_ptr) : node_(node), set_ptr_(set_ptr) {}
  ...
  Set() : size_(0), root_(nullptr), min_node_(nullptr), max_node_(nullptr) {}
  ...
  T &operator[](const T &key) {
    RBTreeNode *res = FindNode(key);
    if (res == nullptr) {
      res = InsertNode(key).first;
      size_++;
      if (min_node_ == nullptr || KeyCompare()(res->data_, min_node_->data_)) {
        min_node_ = res;
      }
    }
    return res->data_;
  ...
  Pair<iterator, bool> insert(const T &value) {
    ...
    if (res.second) {
      size_++;
      if (min_node_ == nullptr || KeyCompare()(value, min_node_->data_)) {
        min_node_ = res.first;
      }
      if (max_node_ == nullptr || KeyCompare()(max_node_->data_, value)) {
        max_node_ = res.first;
      }
    }
